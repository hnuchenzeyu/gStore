...
{
  if (this->value == true_value)
    return false_value;
  if (this->value == false_value)
    return true_value;
...
FilterEvaluationMultitypeValue::EffectiveBooleanValue
FilterEvaluationMultitypeValue::EffectiveBooleanValue::
operator||(const EffectiveBooleanValue& x)
{
  if (this->value == true_value && x.value == true_value)
    return true_value;
  if (this->value == true_value && x.value == false_value)
    return true_value;
  if (this->value == false_value && x.value == true_value)
    return true_value;
  if (this->value == false_value && x.value == false_value)
    return false_value;
  if (this->value == true_value && x.value == error_value)
    return true_value;
  if (this->value == error_value && x.value == true_value)
    return true_value;
  if (this->value == false_value && x.value == error_value)
    return error_value;
  if (this->value == error_value && x.value == false_value)
    return error_value;
  if (this->value == error_value && x.value == error_value)
    return error_value;
...
FilterEvaluationMultitypeValue::EffectiveBooleanValue
FilterEvaluationMultitypeValue::DateTime::
operator==(const DateTime& x)
{
  if (this->date == x.date)
    return FilterEvaluationMultitypeValue::EffectiveBooleanValue::true_value;
  else
    return FilterEvaluationMultitypeValue::EffectiveBooleanValue::false_value;
}
...
FilterEvaluationMultitypeValue
FilterEvaluationMultitypeValue::
operator||(FilterEvaluationMultitypeValue& x)
{
  FilterEvaluationMultitypeValue ret_femv;
  ret_femv.datatype = xsd_boolean;
  ret_femv.bool_value = EffectiveBooleanValue::error_value;
...
int TempResult::findLeftBounder(const vector<int>& this_pos, const ResultPair& x, const int x_id_cols, const vector<int>& x_pos) const
{
  ...
  while (l < r) {
    int m = (l + r) / 2;
    if (compareRow(this->result[m], this_id_cols, this_pos, x, x_id_cols, x_pos) >= 0)
      r = m;
    else
      l = m + 1;
  }
...
void TempResult::getFilterString(QueryTree::GroupPattern::FilterTree::FilterTreeNode::FilterTreeChild& child, FilterEvaluationMultitypeValue& femv, ResultPair& row, int id_cols, StringIndex* stringindex)
{
  if (child.node_type == QueryTree::GroupPattern::FilterTree::FilterTreeNode::FilterTreeChild::String_type) {
    ...
    if (femv.term_value[0] == '"' && (femv.term_value.find("^^<http://www.w3.org/2001/XMLSchema#dateTime>") != string::npos || femv.term_value.find("^^<http://www.w3.org/2001/XMLSchema#date>") != string::npos)) {
...
TempResult::matchFilterTree(QueryTree::GroupPattern::FilterTree::FilterTreeNode& filter, ResultPair& row, int id_cols, StringIndex* stringindex)
{
  ...
  if (filter.oper_type == QueryTree::GroupPattern::FilterTree::FilterTreeNode::Builtin_isiri_type || filter.oper_type == QueryTree::GroupPattern::FilterTree::FilterTreeNode::Builtin_isuri_type) {
    FilterEvaluationMultitypeValue x;
  ...
  if (filter.oper_type == QueryTree::GroupPattern::FilterTree::FilterTreeNode::Builtin_isliteral_type) {
    ...
    ret_femv.bool_value = (x.datatype == FilterEvaluationMultitypeValue::literal || x.datatype == FilterEvaluationMultitypeValue::xsd_string || x.datatype == FilterEvaluationMultitypeValue::xsd_boolean || x.datatype == FilterEvaluationMultitypeValue::xsd_integer || x.datatype == FilterEvaluationMultitypeValue::xsd_decimal || x.datatype == FilterEvaluationMultitypeValue::xsd_float || x.datatype == FilterEvaluationMultitypeValue::xsd_double || x.datatype == FilterEvaluationMultitypeValue::xsd_datetime);
  ...
  if (filter.oper_type == QueryTree::GroupPattern::FilterTree::FilterTreeNode::Builtin_isnumeric_type) {
    ...
    ret_femv.bool_value = (x.datatype == FilterEvaluationMultitypeValue::xsd_integer || x.datatype == FilterEvaluationMultitypeValue::xsd_decimal || x.datatype == FilterEvaluationMultitypeValue::xsd_float || x.datatype == FilterEvaluationMultitypeValue::xsd_double);
