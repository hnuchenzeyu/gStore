...
public:
    ...
    Bind(const std::string& _str, const std::string& _var)
        : str(_str)
        , var(_var)
    {
    }
...
public:
    ...
    SubGroupPattern(SubGroupPatternType _type)
        : type(_type)
    {
    }
    SubGroupPattern(const SubGroupPattern& _sgp)
        : type(_sgp.type)
    {
...
public:
  ...
  public:
  QueryTree()
      : query_form(Select_Query)
      , projection_modifier(Modifier_None)
      , projection_asterisk(false)
      , offset(0)
      , limit(-1)
      , update_type(Not_Update)
  {
  }
